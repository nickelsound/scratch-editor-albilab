...
Herder will then resolve or reject based on the outcome of the task.

## Usage

```javascript
import TaskHerder from 'task-herder'

const herd = new TaskHerder({
...
herd
  .do(
    ...
    {
      // Provide the signal to the task herder so it can cancel the task before it starts
      signal: controller.signal,
    },
  )
  .catch(error => {
...
```javascript
const taskPromise = herd.do(() => fetch('https://example.com/data'))
herd.cancel(taskPromise, optionalReason) // Remove a specific task from the queue
herd.cancelAll(optionalReason) // Remove all tasks from the queue
```
